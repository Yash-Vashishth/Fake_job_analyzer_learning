...
# Once this many NEW labels have come in since the last training run, the
# next /flag call automatically triggers a retrain — no manual /retrain
# or cron job needed for the common case. Set higher if you want more
# labeled data to accumulate between adjustments; lower for faster (but
# noisier) adaptation.
...
def maybe_auto_retrain() -> dict:
    """
    Call this after every new label. Triggers a real retrain() only once
    enough NEW labeled data has accumulated since the last training run
    (or on the first run, once the bootstrap minimum is reached). Cheap
    to call on every /flag — it's just a count comparison unless it
    actually decides to retrain.

    Returns {"triggered": False, ...} or the full retrain() result with
    "triggered": True mixed in.
    """
    status = current_status()
    total = status["total_labeled_submissions"]

    last_trained_count = 0
    if os.path.exists(LEARNED_WEIGHTS_PATH):
        try:
            with open(LEARNED_WEIGHTS_PATH) as f:
                last_trained_count = json.load(f).get("total_labeled_submissions", 0)
        except Exception:
            pass

    new_since_last = total - last_trained_count
    ready = total >= MIN_SAMPLES_PER_PARAM and new_since_last >= AUTO_RETRAIN_EVERY_N_NEW_LABELS

    if not ready:
        return {
            "triggered": False,
            "total_labeled_submissions": total,
            "new_since_last_train": new_since_last,
            "needed_for_next_retrain": max(0, AUTO_RETRAIN_EVERY_N_NEW_LABELS - new_since_last),
        }
